...
knitr::opts_chunk$set(
  ...
  fig.height = 4.5,
  dpi = 110,
  eval = identical(Sys.getenv("BAYESQM_BUILD_VIGNETTE"), "true")
)
```
...
<details>
<summary>Building this vignette with live output</summary>
...
On CRAN machines the variable is unset, so the document compiles
quickly without a Stan backend.
</details>
...
vector of forced-distribution counts.
...
For real studies, `read_qsort()` auto-detects the file format:
...
prior on the loadings, then resolves rotational ambiguity with
MatchAlign.
...
Convergence statistics are stored on pre-alignment draws and printed
in the fit header. They are also available programmatically:
...
every factor. `plot_loading_posterior()` draws the canonical
loading forest, with nested 50 % and 95 % credible-interval whiskers
and the classical Brown cut-off `1.96 / sqrt(J)` as a faint
...
For a single statement across all factors, `plot_zscore_posterior()`
...
**peak-plus-Sivula** protocol: the ELPD peak is the automated
choice; the Sivula (2025) parsimony rule is reported alongside.
...
```{r elpd, fig.cap = "ELPD across K. Solid = peak (automated choice). Dashed = Sivula."}
plot_elpd(run)
```

The `run$case` field labels this relationship. When both methods
pick the same K, the case is `agree` and the data supports that K
unambiguously. When the ELPD peak exceeds the Sivula choice, the
case is `gap`: the richer model fits a bit better on LOO but not
...
`bayesqm` replaces the classical z-score test with a direct
posterior probability. For every statement and every factor pair,
...
`T × P` Stan-style draws matrix that `posterior` and `bayesplot`
consume natively.
