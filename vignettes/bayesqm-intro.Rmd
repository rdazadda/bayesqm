...
above 400, and zero divergent transitions [@VehtariEtAl2021]:
...
reported alongside the peak so readers can see how confidently the
data discriminate between adjacent models. The `run$case` field
labels the relationship between the two: when Sivula lands on the
same K as the peak, the case is `agree`; when Sivula points to a
smaller K than the peak, the case is `gap`; when Sivula exceeds the
peak, the case is `reversed` and usually indicates numerical
instability in the comparison itself. In every case the adopted K is
the ELPD peak.
